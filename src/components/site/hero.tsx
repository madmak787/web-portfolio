...
export default function Hero() {
	return (
		<section id="home" className="relative overflow-hidden pb-12 pt-24 sm:pt-32 md:pb-16 md:pt-40">
			...
			<div className="container relative grid items-center gap-12 lg:grid-cols-[1.15fr_0.85fr] lg:gap-16">
				<div className="min-w-0">
					<div className="reveal mb-6 inline-flex items-center gap-2 rounded-full border bg-surface/80 py-1.5 pl-2 pr-3.5 text-xs font-medium text-muted backdrop-blur">
					...
					<p className="reveal eyebrow mb-4 leading-relaxed" style={{ "--delay": "60ms" } as React.CSSProperties}>
						{profile.name} — {profile.role}
					</p>

					<h1 className="reveal font-display text-[2.5rem] font-semibold leading-[1.04] tracking-tight sm:text-6xl xl:text-7xl" style={{ "--delay": "120ms" } as React.CSSProperties}>
			...
			<div className="container relative mt-14 md:mt-20">
				<dl className="reveal grid grid-cols-2 overflow-hidden rounded-2xl border bg-line/[0.09] md:grid-cols-4" style={{ gap: "1px" }}>
					{stats.map((s) => (
						<div key={s.label} className="bg-surface px-5 py-6 sm:px-7">
							<dt className="text-xs font-medium text-muted sm:text-sm">{s.label}</dt>
							<dd className="mt-1 font-display text-3xl font-semibold tracking-tight sm:text-4xl">{s.value}</dd>
						</div>
					))}
				</dl>
			</div>
...
function ProfileCard() {
	...
	return (
		<div className="reveal relative mx-auto hidden w-full min-w-0 max-w-md sm:block lg:max-w-none" style={{ "--delay": "200ms" } as React.CSSProperties}>
			...
			<div className="card relative overflow-hidden shadow-2xl shadow-black/10">
				...
				<pre className="no-scrollbar overflow-x-auto p-5 font-mono text-[12.5px] leading-7 sm:text-[13px]">
					<code>
